...
for sc in [int(x) for x in sys.argv[1:]]:
    ...
    if len(blob) <= size:
        ...
        # 확장 컨테이너 폴백: 현행(DAT_jp_final) 컨테이너의 sce 슬롯에 스왑 (2026-07-29 방식)
        nf2 = struct.unpack_from("<I", dat, b2 + 8)[0]
        sz2 = [struct.unpack_from("<I", dat, b2 + 16 + 4 * k)[0] for k in range(nf2)]
        ccont = bytes(dat[b2:b2 + 16 + 4 * nf2 + sum(sz2)])
        _, off2, size2, _ = next(bb for bb in parse_blobs(ccont) if bb[0] == "sce")
        if len(blob) > size2:
            print(f"[FAIL] scene {sc}: over by {len(blob)-size2}B vs 확장슬롯 ({how})")
            continue
        assert b2 + len(ccont) <= dp[sc + 1]
        dat[b2 + off2:b2 + off2 + size2] = blob + b"#" * (size2 - len(blob))
        print(f"[OK-EXP] scene {sc} (blob {len(blob)}/{size2}B 확장슬롯, {how})")
